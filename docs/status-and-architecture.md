...
| `profiles` | **Absent / placeholder** — not an active dependency | No reference in any `supabase/functions/` or `supabase/migrations/` file (confirmed via full-repo grep). The only occurrence of `profiles` as a table name anywhere in the repo is a test-connection snippet in `CLAUDE.md` (already flagged as stale pasted documentation — see §4 item 4), not evidence of a live schema dependency. |
...
- Version-controlled, current, and mutually consistent base schema for `job_applications` and `resume_versions` (existing legacy DDL is stale/incompatible; `profiles` has no active dependency to recover).
