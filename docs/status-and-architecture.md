...
At the same time, the repo confirms the caution both Eric and ChatGPT raised, though not for the reason first stated: the **base schema has no current, canonical, replayable source of truth in `supabase/migrations/`** — not because the DDL is entirely absent, but because what's checked in is stale and mutually inconsistent. `sql/setup-job-automation-tables.sql` creates `job_applications` with `job_url`/`ai_relevance_score` columns, but the active promotion code (`supabase/functions/_shared/scoring.ts`) upserts `url`/`ai_match_score` instead (`onConflict: 'url'`). `database-ai-updates.sql` creates `resume_versions` with `job_id INTEGER`, incompatible with the UUID `job_applications.id` values that `process-ready-jobs/index.ts` actually filters `resume_versions.job_id` against. `profiles` has no active function or `supabase/migrations/` reference and is therefore not established as a current active dependency by repository evidence — though historical/stale material describes it in multiple places: `PROGRESS.md` (from the repo's earliest commit) lists it among "Database schema implemented"; `archive/job-search-PRD.md` (explicitly moved to `archive/` in the same commit that retired n8n) contains a DDL-like `table profiles (...)` declaration; and `CLAUDE.md` (§4 item 4) contains a test-connection snippet. None of these establish that the table currently exists or is operational. `AUTOMATION.md` states directly that the database was **restored** at some point and that "the checked-in migrations are incremental against the restored application schema; they are not a complete empty-database bootstrap." That remains repo-confirmed evidence of a prior incident, not just historical hearsay — see finding 1 in §4.
...
|---|---|---|
| `job_applications`, `resume_versions` base tables | **Partially implemented** — stale, mutually inconsistent legacy DDL; no current canonical migration | Legacy `CREATE TABLE` statements exist outside `supabase/migrations/`: `sql/setup-job-automation-tables.sql` defines `job_applications` with `job_url`/`ai_relevance_score`, but active code (`supabase/functions/_shared/scoring.ts` L306–330) upserts `url`/`ai_match_score` instead. `database-ai-updates.sql` defines `resume_versions` with `job_id INTEGER`, incompatible with the UUID `job_applications.id` values that `process-ready-jobs/index.ts` (L42–60) filters `resume_versions.job_id` against. Neither file is a `supabase/migrations/` entry, so no single current, replayable schema exists for either table. `AUTOMATION.md` L85–88: *"The restored database data does not recreate platform-level resources... Restore/create the base tables before running `supabase db push` on a brand-new project."* confirms the live schema is not fully captured in git. |
| `profiles` | **Absent / placeholder** — not established as active; legacy documentation only | No reference in any active `supabase/functions/` or `supabase/migrations/` file (confirmed via full-repo grep). Historical materials describe it: `PROGRESS.md` lists it in the reported schema from the repo's earliest commit; `archive/job-search-PRD.md` contains a DDL-like `table profiles (...)` declaration; `CLAUDE.md` (§4 item 4) contains a test-connection snippet. None of these establish that the table currently exists or is operational. |
...
| `sql/setup-job-automation-tables.sql` | **Partially implemented** — stale relative to active code; not formally archived | Defines `job_raw` with columns (`source_url`, `source_type`) that don't match the column the current code actually dedupes on (`job_url`). Also defines `job_applications` with `job_url`/`ai_relevance_score`, conflicting with the `url`/`ai_match_score` columns `_shared/scoring.ts` actually writes (see the base-tables row above). Repo history does not support an "archived" label: commit `760af84` — the same commit that archived n8n material to `archive/n8n/` — edited this file in place rather than retiring it. |
...
| `plan-submission` (Phase 0 auto-apply router) | Classify each active job into `api`/`email`/`assisted`/`manual`; write a submission plan; **zero outward actions** | **Reported implemented, runtime unverified** | `supabase/functions/plan-submission/index.ts`; `supabase/functions/_shared/submission-plan.ts` + `submission-plan_test.ts` (defines five `Deno.test` cases: mailto→email, Workday URL→assisted with `api_authorized:false`, missing URL→manual, malformed URL→manual, auth-metadata-before-master-resume; current runtime result unverified — no checked-in execution result was found, consistent with the absence of CI noted in §3.4). Commit `c46664a`. Doc self-status: *"Phase 0 implemented and PostgreSQL-verified locally; not yet deployed."* |
...
| Deployment mechanism | **Manual** | `AUTOMATION.md`'s "One-Time Deployment" section is a checklist of manual `supabase` CLI commands (`link`, `secrets set` ×8 — four required plus four optional, `functions deploy` ×7, `db push`), run by a person, not a pipeline. |
...
1. **Base schema has no current, canonical source of truth in `supabase/migrations/`.** Checked-in `CREATE TABLE` DDL for `job_applications` (`sql/setup-job-automation-tables.sql`) and `resume_versions` (`database-ai-updates.sql`) does exist, but both are stale and mutually inconsistent with active code and with each other — see §3.1. `profiles` has no active reference and is not established as a current dependency, though it appears in historical material (`PROGRESS.md`, `archive/job-search-PRD.md`, `CLAUDE.md` — see §3.1). If the Supabase project were lost again, the migrations in `supabase/migrations/` plus these two legacy files would not reliably rebuild a working schema without manual reconciliation. `AUTOMATION.md` acknowledges the restore/rebuild gap directly rather than hiding it, which is good, but it means the "Supabase as source of truth" principle currently has a single point of failure with no clean version-controlled recovery path.
...
| Repository code (`_shared/*.ts`) | Deterministic policy, validation, and state-transition logic, combined with OpenAI-backed scoring and tailoring | `_shared/submission-plan.ts` and `_shared/application-replies.ts` are deterministic routing/classification logic with no LLM calls. `_shared/scoring.ts` and `_shared/tailor.ts` call OpenAI directly for the scoring/tailoring steps themselves — not wholly deterministic (scoring has a deterministic keyword fallback when no API key is set; tailoring does not). All four live here rather than in the Edge Function entrypoints. |
...
| LLM integration | Scoring, tailoring, extraction, classification | **OpenAI-specific today, not provider-agnostic:** `_shared/scoring.ts`, `_shared/tailor.ts`, `ingest-email-jobs/index.ts`, and `ingest-application-replies/index.ts` all call `https://api.openai.com/v1/chat/completions` directly. `AI_SCORING_MODEL` (default `gpt-5.4-nano`) and `TAILORING_MODEL` (default `gpt-4o-mini`) select OpenAI model names, not providers. Anthropic appears only in `AI-INTEGRATION-GUIDE.md`'s design examples; no active Anthropic adapter or call path exists. A provider-neutral adapter is a planned, unbuilt future option. |
...
- **Browser (tracker UI) ↔ Supabase:** anon key + user session; the checked-in `tighten_rls` migration is intended to enforce owner-only access if it has been applied. No OpenAI key ever reaches the browser (`AI-INTEGRATION-GUIDE.md`); no active Anthropic key or call path exists to reach it in the first place.
- **Edge Functions ↔ OpenAI:** service-role only, secrets (`OPENAI_API_KEY`) stored in Supabase Vault/secrets, never in the repo (`AGENTS.md` explicitly warns about GitHub Push Protection for this reason). No active Anthropic call path exists; Anthropic appears only in `AI-INTEGRATION-GUIDE.md`'s design examples.
...
| OpenAI | Scoring, tailoring, extraction, classification | Active: `_shared/scoring.ts`, `_shared/tailor.ts`, `ingest-email-jobs`, and `ingest-application-replies` all call `https://api.openai.com/v1/chat/completions` directly with `OPENAI_API_KEY`; whether the secret is currently set and calls succeed at runtime is unverified (§8) | Scoring falls back to deterministic keyword matching when no API key is set; tailoring has no fallback and requires OpenAI. Quarantine generation work on failure. A provider-neutral adapter is a planned, unbuilt future option. |
...
- Version-controlled, current, and mutually consistent base schema for `job_applications` and `resume_versions` (existing legacy DDL is stale/incompatible; `profiles` is not established as active — see §3.1 — so there is no active dependency to recover).

---

## 7. Phased roadmap

1. **Verify and reactivate the existing runtime.** Execute §8's read-only checks (items 1–6, 8–10) first and record evidence. Item 7 is a separate, optional, owner-authorized mutating smoke test — run it only with Eric's explicit authorization, and never as part of a documentation-only pass. Then repair only failed components with explicit before/after results.
...
Run these to convert "reported implemented" into "verified operational" for each layer. Items 1–6 and 8–10 are read-only: they confirm, list, count, or inspect existing state and make no external calls that create, modify, or send anything. Item 7 is different in kind — it is an **optional, explicitly authorized mutating smoke test**, not a read-only check, and must not be run as part of a documentation-only correction or an independent evidence review.
...
6. **Base tables exist with data:** `select count(*) from job_applications;` / `resume_versions;` — nonzero counts and current timestamps indicate a live, non-stale system. (`profiles` is not an active dependency — see §3.1 — and does not need to be checked here.)
7. **Manual smoke test (mutating — requires separate owner authorization):** invoking `discover-jobs` fetches the configured external RSS/ATS feeds; it may upsert newly discovered records into `job_raw` (only if new, non-duplicate entries were found); it may call OpenAI, incurring API cost, when `OPENAI_API_KEY` is configured and newly inserted data reaches the scoring stage; and it may promote/upsert qualifying records into `job_applications` (only for candidates scoring at or above the configured threshold). Only Eric may authorize running this, and only outside the scope of a documentation or evidence-review pass. If run: invoke from the Supabase dashboard or a local shell that reads `CRON_SECRET` from a pre-set environment variable. Do not place the secret value in this document, chat, command history, screenshots, or captured output. Expect a JSON summary with `fetched`, `inserted_raw`, `filtered_out`, and `promoted_to_review`.
