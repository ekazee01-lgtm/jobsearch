...
Every component below gets exactly one status tag. A tag describes what the **evidence supports**, not what any prior chat (including me) previously asserted.
...
| `data/jobs.json` | **Placeholder, not live data** | Contains only two fixture rows ("Example Corp", "Acme Inc"). Not wired to any function. Real data, if any, is in Supabase — not this file. |
...
| Phase 1B (auto-apply status change for high-confidence rejections) | **Planned** | `docs/phase1b-backlog.md` lists this as unbuilt hardening work "to fold into 1B." No `apply_reply_status_change` RPC found in any migration. |
| Local browser worker (assisted-lane form fill + submit) | **Planned** | Fully specified in `docs/auto-apply-design.md` (browser extension MV3 or desktop companion, long-polls Supabase, stops before final submit). No extension manifest, no companion-app code, nothing outside the design doc found anywhere in the repo. |
| Authorized-API submission lane (`submit-via-api`) | **Planned, and expected to stay empty** | Named in the design doc as a future Edge Function; not present in `supabase/functions/`. The design doc itself states this lane "will normally be empty" for an applicant-owned system — see §6. |
...
| Live hosting | **Reported implemented, runtime unverified** | `README.md` and `AUTOMATION.md` both cite `https://ekazee01-lgtm.github.io/jobsearch/tracker.html` as the live URL. Whether GitHub Pages is currently serving the current commit is unverified from a clone. |
...
| Component | Status | Evidence |
|---|---|---|
| GitHub Actions (any workflow) | **Not present** | No `.github/` directory anywhere in the repository tree (confirmed by full-tree search). GPT's proposed "CI/deployment + exceptional batch work only" role for GitHub Actions is a design decision to make going forward — it doesn't describe anything that exists today. |
...
| `validate.sh` (referenced in `CLAUDE.md`) | **Referenced but does not exist** | `CLAUDE.md` tells future Claude sessions to run `./validate.sh`; no such file exists in the repo. `CLAUDE.md` also opens with the literal text *"You're right - let me provide the enhanced Claude.md file directly."* — internal evidence this file is a pasted AI response, not curated documentation, and its "Last Updated: November 6, 2024" predates the repo's actual first commit (2025-11-03). Treat `CLAUDE.md` as stale/unreliable relative to `AUTOMATION.md`, `README.md`, and the dated docs in `docs/`. |
...
| Reply-selection filter strategy | **Reported active, unverified** | `docs/email-ingest.md`: *"Currently active: plus-addressing (option 1)"* — i.e., `ekazee01+apply@gmail.com` as the reply-catching address. This is a Gmail account setting, not repo state — unverifiable here. |
...
3. **Three stale/superseded SQL files remain at the repo root and in `sql/`** (`interview-scheduling-schema.sql`, `sql/setup-job-automation-tables.sql`, plus `database-staging-setup.sql` and `sql/application-materials-corrected.sql`, `sql/debug-tables.sql`, unreviewed in detail here) that reference older table shapes. None are wired into the active `AUTOMATION.md` runbook. Low risk, but they will confuse the next person (human or AI) who greps the repo for schema definitions, exactly as they nearly confused this review.
...
| Local browser worker | Browser-assisted form completion and submission | **Design-only** (§3.2). Not built. |
...
```mermaid
flowchart TD
    A1["RSS/Atom feeds (9 sources)"] --> N["job_raw (normalize + dedupe)"]
    ...
    W --> Receipt["Receipt: timestamp, confirmation ref,\nmaterials version, run id"]
...
- **Browser (tracker UI) ↔ Supabase:** anon key + user session; RLS enforces owner-only access post-`tighten_rls` migration. No OpenAI/Anthropic key ever reaches the browser (`AI-INTEGRATION-GUIDE.md`).
...
- **Apps Script ↔ Edge Functions:** shared-secret header (`x-cron-secret`) read from Script Properties at runtime, not committed. Runs entirely inside Eric's own Google account — no third-party OAuth app, no token expiry (`docs/email-ingest.md`).
...
- Local browser worker (form-fill + submit) — design complete, zero code.
...
## 7. Verification checklist (Eric — requires dashboard/service-role access this session doesn't have)
...
7. **Manual smoke test:** `curl -X POST https://hndkhpwzvybbiagnjkdr.supabase.co/functions/v1/discover-jobs -H "x-cron-secret: <value>"` — per `AUTOMATION.md`'s own verification section, expect a JSON summary with `fetched`, `inserted_raw`, `filtered_out`, `promoted_to_review`.
8. **GitHub Pages:** load `https://ekazee01-lgtm.github.io/jobsearch/tracker.html` directly and confirm it renders and authenticates.
...
## 8. Proposed next planning document

A **"Runtime Verification & Reactivation Plan"** — walks the checklist in §7, records pass/fail for each item with actual output (not just intent), and for every failing item specifies the exact fix (recreate secret, redeploy function, reschedule cron, re-authorize Apps Script trigger). That plan turns this document's "reported implemented" rows into "verified operational" rows one at a time, and is the correct prerequisite before designing or building anything new — including the local browser worker, which depends on `plan-submission` actually running in production first.
